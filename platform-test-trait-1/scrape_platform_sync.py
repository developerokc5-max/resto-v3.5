...
def check_skip_scheduled_run():
    ...
    5 minutes, skip this run to avoid double-scanning and wasting Actions minutes.
    ...
    try:
        ...
        if row:
            log(f"⏭ Skipping scheduled run — manual scan was triggered at {row[0]} (within last 5 minutes). Resuming normal schedule next run.")
            sys.exit(0)
    except Exception as e:
...
def select_organization_group(page):
    ...
    try:
        ...
        try:
            screenshot_path = os.path.join(SCRIPT_DIR, "debug_after_dropdown_click.png")
            page.screenshot(path=screenshot_path)
        except:
            pass
        ...
        try:
            screenshot_path = os.path.join(SCRIPT_DIR, "debug_after_group_tab.png")
            page.screenshot(path=screenshot_path)
        except:
            pass
        ...
        try:
            screenshot_path = os.path.join(SCRIPT_DIR, "debug_after_org_selected.png")
            page.screenshot(path=screenshot_path)
        except:
            pass
        ...
        try:
            screenshot_path = os.path.join(SCRIPT_DIR, "debug_org_selection_error.png")
            page.screenshot(path=screenshot_path)
        except:
            pass
...
def scan_store_toggles(page):
    ...
    try:
        ...
        screenshot_path = os.path.join(SCRIPT_DIR, "debug_scan_error.png")
        page.screenshot(path=screenshot_path)
        return {}
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                # Insert new platform status data
                total_saved = 0
                ...
                for shop_id, shop_data in result['shops'].items():
                    for platform_name in ['grab', 'foodpanda']:
                        ...
                        try:
                            ...
                            log(f"  ⚠ Error saving {shop_data['name']} [{platform_name}]: {e}")

                db.commit()
                log(f"✓ Saved {total_saved} platform status records")
            ...
            screenshot_path = os.path.join(SCRIPT_DIR, "debug_fatal_error.png")
            page.screenshot(path=screenshot_path)
