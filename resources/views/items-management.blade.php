...
<head>
    ...
    <style>
        ...
        .item-row:hover { background-color: #f7fafc; }
        .dark .item-row:hover { background-color: #334155; }
        .status-badge-available { background-color: #c6f6d5; color: #22543d; }
        .status-badge-unavailable { background-color: #fed7d7; color: #742a2a; }
        .dark .status-badge-available { background-color: rgba(34,197,94,0.2); color: #86efac; }
        .dark .status-badge-unavailable { background-color: rgba(239,68,68,0.2); color: #fca5a5; }
    </style>
</head>
<body class="bg-gray-50 dark:bg-slate-950 dark:text-gray-100 transition-colors duration-200">
    <!-- Navigation -->
    <nav class="bg-white dark:bg-slate-900 shadow-lg border-b-4 border-blue-500">
            ...
            <div class="flex justify-between h-16">
                <div class="flex items-center space-x-8">
                    ...
                    <div class="hidden md:flex space-x-4">
                        ...
                        <a href="/items/management" class="text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-2 rounded-md text-sm font-medium">
    ...
    <!-- Header -->
    <div class="bg-white dark:bg-slate-900 border-b dark:border-slate-800 px-4 sm:px-6 lg:px-8 py-6">
        <div class="max-w-7xl mx-auto">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">
                <i class="fas fa-cog mr-2"></i>Items Management
            </h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Monitor menu items across all platforms</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="text" id="searchInput" placeholder="Search items..."
                        class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 dark:placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Shop</label>
                    <select id="shopFilter" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <select id="categoryFilter" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <select id="statusFilter" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead class="bg-gray-50 dark:bg-slate-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Grab</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">FoodPanda</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Deliveroo</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Status</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700" id="itemsTableBody">
                        ...
                        <tr class="item-row"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 flex-shrink-0 bg-gradient-to-br from-blue-100 dark:from-blue-900/30 to-purple-100 dark:to-purple-900/30 rounded-lg flex items-center justify-center">
                                        <i class="fas fa-utensils text-blue-600 dark:text-blue-400"></i>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $itemGroup['name'] }}</div>
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $itemGroup['sku'] }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-gray-100">{{ $itemGroup['shop_name'] }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">${{ number_format($itemGroup['price'], 2) }}</div>
                            </td>
    ...
    <!-- Toast Notification -->
    <div id="toast" class="fixed bottom-4 right-4 bg-white dark:bg-slate-800 rounded-lg shadow-lg p-4 transform translate-y-20 transition-transform duration-300 hidden">
        <div class="flex items-center space-x-3">
            <div id="toastIcon"></div>
            <div id="toastMessage" class="text-sm font-medium text-gray-900 dark:text-gray-100"></div>
        </div>
    </div>
        ...
        async function toggleItemStatus(itemId, isAvailable, platform) {
            ...
            try {
                const response = await fetch('/api/items/toggle-status', {
                    ...
                    body: JSON.stringify({ item_id: itemId, is_available: isAvailable, platform: platform })
                });
